...
def open_file(event=None):
    global url
    url=filedialog.askopenfilename(initialdir=os.getcwd(),title='Select To Open File',filetypes=(('Text Files','*.txt'),("All Files","*.*")))
    try:
        ...
        if text_changed:
            ...
            if mbox is True:
                if url:
                    ...
                    url=filedialog.asksaveasfile( url=filedialog.asksaveasfile(mode='w',defaultextension='.txt',filetypes=(('Text Files','*.txt'),('All Files','*.*'))))
                    url.write(content2)
                    url.close()
                    main_application.destroy()
            elif mbox is False:
